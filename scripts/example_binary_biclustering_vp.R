...
create_data <- function(M, N, R, C, pi_r, kappa_c, theta_rc){
    ...
    for(i in 1:N) {
        ...
        for(j in 1:M){
            ...
            k <- M*(i - 1) + j
            data[k] <- rbinom(1, 1, theta_rc[r, c])
            rows[k] <- i
            cols[k] <- j
            rowclusters[k] <- r
            colclusters[k] <- c
    ...
    long.df <- data.frame(Y = factor(data), ROW = rows, COL = cols, 
...
long.df <- create_data(M, N, R, C, pi_r, kappa_c, theta_rc)
