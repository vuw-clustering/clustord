...
ex_rowclustering <- function(formula, long.df, row.covariate, pi_r){
    ...
    for(r in 1:R){
        for(i in results$RowClusters[[r]]){
            for(j in 1:M){
                ...
                thetas.est[i] <- theta.model
                thetas.true[i] <- theta.exact
    ...
    return(list(theta.mse.error=theta.mse.error, results=results, thetas= data.frame(est=thetas.est, true=thetas.true)))
}
...
case3 <- function(Nvals){
    ...
    for (N in Nvals){
        ...
        for (formula in c("Y~row", "Y~row+row.covariate")) {
            ...
            legend("bottomright", legend = c("est.", "true"), pch = c(4,1))
            dev.off()
...
legend("topright", legend = c('Y~row+row.covariate', 'Y~row'), pch = c(8, 0))
dev.off()
