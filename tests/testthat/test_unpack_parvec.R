test_that("unpack.parvec produces correct results.", {
    ...
    ## OSM ----
    param.lengths <- c(q,q,RG,0,0,0,0,0,0,0,0,0)
    names(param.lengths) <- names.param.lengths
    ...
    param.lengths <- c(q,q,RG,p,0,0,0,0,0,0,0,0)
    names(param.lengths) <- names.param.lengths
    ...
    param.lengths <- c(q,q,RG,p,RG*p,0,0,0,0,0,0,0)
    names(param.lengths) <- names.param.lengths
    ...
    param.lengths <- c(q,q,0,0,RG*p,0,0,0,0,0,0,0)
    names(param.lengths) <- names.param.lengths
    ...
    ## Biclustering
    CG <- 2
    param.lengths <- c(q,q,RG,0,0,0,0,CG,0,0,0,0)
    names(param.lengths) <- names.param.lengths
    ...
    # First, the model with interaction terms and main effects
    param.lengths <- c(q,q,RG,0,0,0,0,CG,0,0,0,RG*CG)
    names(param.lengths) <- names.param.lengths
    ...
    param.lengths <- c(q,q,0,0,0,0,0,0,0,0,0,RG*CG)
    names(param.lengths) <- names.param.lengths
    expect_equal(unpack.parvec(c(1,2,3,-1,1,-1.5,2,1.5,0.5,0.5),model="OSM",
                               param.lengths=param.lengths, n=n, p=p, q=q, RG=RG, CG=2, constraint.sum.zero=TRUE),
                 list(mu=c(0,1,2,3),phi=c(0,expit(-1),expit(-1 + exp(1)),1),
    ...
    ## POM ----
    param.lengths <- c(q,0,RG,0,0,0,0,0,0,0,0,0)
    names(param.lengths) <- names.param.lengths
    ...
    param.lengths <- c(q,0,RG,p,0,0,0,0,0,0,0,0)
    names(param.lengths) <- names.param.lengths
    ...
    param.lengths <- c(q,0,RG,p,RG*p,0,0,0,0,0,0,0)
    names(param.lengths) <- names.param.lengths
    rowc.col <- matrix(1:((RG-1)*(p-1)), nrow=(RG-1), byrow=TRUE)
    rowc.col <- cbind(rowc.col,-rowSums(rowc.col))
    rowc.col <- rbind(rowc.col,-colSums(rowc.col))
    ...
    CG <- 2
    param.lengths <- c(q,0,RG,0,0,0,0,CG,0,0,0,0)
    names(param.lengths) <- names.param.lengths
    ...
    param.lengths <- c(q,0,RG,0,0,0,0,CG,0,0,0,RG*CG)
    names(param.lengths) <- names.param.lengths
